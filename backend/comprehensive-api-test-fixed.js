...
async function runComprehensiveAPITest() {
    ...
    totalTests++;
    const contactsListTest = await testEndpoint('GET', '/api/contacts', null, authHeaders);
    if (contactsListTest.success) passedTests++; else failedTests++;

    totalTests++;
    const contactCreateTest = await testEndpoint('POST', '/api/contacts', {
        name: 'Test Contact',
        phone: '9876543210',
    ...
    totalTests++;
    const bulkTextTest = await testEndpoint('POST', '/api/contacts/bulk-text', {
        contacts: [
    ...
    totalTests++;
    const campaignsListTest = await testEndpoint('GET', '/api/campaigns', null, authHeaders);
    if (campaignsListTest.success) passedTests++; else failedTests++;

    totalTests++;
    const campaignCreateTest = await testEndpoint('POST', '/api/campaigns', {
        name: 'Test Campaign',
    ...
    totalTests++;
    const devicesListTest = await testEndpoint('GET', '/api/devices', null, authHeaders);
    if (devicesListTest.success) passedTests++; else failedTests++;

    totalTests++;
    const deviceRegisterTest = await testEndpoint('POST', '/api/devices/register', {
    ...
    totalTests++;
    const analyticsTest = await testEndpoint('GET', '/api/analytics/dashboard', null, authHeaders);
    if (analyticsTest.success) passedTests++; else failedTests++;

    totalTests++;
    const analyticsTestEndpoint = await testEndpoint('GET', '/api/analytics/test', null, authHeaders);
    ...
    totalTests++;
    const callLogsTest = await testEndpoint('GET', '/api/call-logs', null, authHeaders);
    ...
    totalTests++;
    const schedulesTest = await testEndpoint('GET', '/api/schedules', null, authHeaders);
    ...
    totalTests++;
    const unauthorizedTest = await testEndpoint('GET', '/api/auth/me', null, {}, 401);
    if (unauthorizedTest.success) passedTests++; else failedTests++;

    totalTests++;
    const notFoundTest = await testEndpoint('GET', '/api/nonexistent-endpoint', null, authHeaders, 404);
